...
const blockMatch = text.match(/<h3 className="font-bold mb-2">Änderungen v0\.1\.0-dev:<\/h3>\s*<ul className="space-y-1 text-gray-300">\s*(?:<li>.*?<\/li>\s*)*<\/ul>/);

if (blockMatch) {
    const oldChunk = blockMatch[0];
    const liRegex = /(\s*<li>.*?<\/li>)/g;
    const lis = [];
    let match;
    while ((match = liRegex.exec(oldChunk)) !== null) {
        lis.push(match[1]);
    }
    lis.reverse();
    const newChunk = '<h3 className="font-bold mb-2">Änderungen v0.1.0-dev:</h3>\n                 <ul className="space-y-1 text-gray-300">' + lis.join('') + '\n                 </ul>';
