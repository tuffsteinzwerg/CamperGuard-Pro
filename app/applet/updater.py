...
"""
text = text.replace('<h3 className="font-bold mb-2 text-[var(--accent)]">Änderungen v0.1.3-dev:</h3>', new_block + '<h3 className="font-bold mb-2 text-[var(--accent)]">Änderungen v0.1.3-dev:</h3>')

with open('old_list.txt', 'r', encoding='utf-8') as f:
    old_list_lines = f.readlines()

new_list_lines = []
for l in reversed(old_list_lines):
    if l.strip().startswith('<li>'):
        new_list_lines.append(l)

new_list_text = "".join(new_list_lines)
old_list_text = "".join(old_list_lines)

# Wait, `old_list.txt` contains lines including the start/end lines since I used sed -n "/Änderungen.../,/<\/ul>/p".
# Actually, I should just replace the list.
# Let's extract the list exactly.
with open('src/App.tsx', 'r', encoding='utf-8') as f:
    t = f.read()
    
match = re.search(r'<h3 className="font-bold mb-2">Änderungen v0.1.0-dev:</h3>\s*<ul className="space-y-1 text-gray-300">\s*(<li>.*?</li>\s*)*</ul>', text, re.DOTALL)
if match:
    old_chunk = match.group(0)
    # extract all <li>...</li>
    li_matches = re.findall(r'(\s*<li>.*?</li>\s*)', old_chunk)
    # reverse
    li_matches.reverse()
    
    # re-assemble
    new_chunk = '<h3 className="font-bold mb-2">Änderungen v0.1.0-dev:</h3>\n                 <ul className="space-y-1 text-gray-300">\n' + "".join(li_matches) + '                 </ul>'
    text = text.replace(old_chunk, new_chunk)
else:
    print("Could not find v0.1.0-dev block")
